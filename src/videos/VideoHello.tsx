...
function Text() {
	...
	const opacity = interpolate(frame, [0, durationFade, durationInFrames - durationFade, durationInFrames], [0, 1, 1, 0]);
	const shift = Math.sin(2 * frame / fps) * 15;
	const rotation = Math.sin(1.25 * frame / fps) * 10;
	return (
		<div style={{opacity, transform: `translateY(${shift}%) rotate(${rotation}deg)`}} className='text-[172px] text-emerald-500'>
			Hello!
		</div>
	);
}
